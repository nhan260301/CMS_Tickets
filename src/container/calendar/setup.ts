...
export function beforeStartDayOfMonth(value: any, day: any) {
  ...
  return (
    day.isBefore(startDayOfMonth, 'day') | day.isAfter(endDayOfMonth, 'day')
  )
}
...
export function isSelectedStartAndEnd(value: any, day: any) {
  return (
    value.clone().isoWeekday(1).isSame(day, 'day') |
    value.clone().isoWeekday(7).isSame(day, 'day')
  )
}
...
export function formatMonthAndYear(value: any) {
  return (
    'Tháng ' +
    value.format('M').toString() +
    ', ' +
    value.format('Y').toString()
  )
}

export function formatDayMonthYear(value: any) {
  return value.format('DD/MM/YYYY').toString()
}
